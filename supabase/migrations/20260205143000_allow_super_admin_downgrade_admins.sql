-- Allow SUPER_ADMIN to downgrade ADMIN or SUPER_ADMIN (with last SUPER_ADMIN guard)
...
BEGIN
  ...
  -- RULE 0: Only SUPER_ADMIN can downgrade/remove ADMIN or SUPER_ADMIN roles
  IF v_target_current_role IN ('admin'::app_role, 'super_admin'::app_role)
     AND p_new_role <> v_target_current_role
     AND v_current_role <> 'super_admin'::app_role THEN
    RETURN jsonb_build_object(
      'success', false,
      'error', 'Only SUPER_ADMIN can downgrade or remove ADMIN or SUPER_ADMIN roles'
    );
  END IF;
